...
def main():
    ...
    for start_point_nr, start_point in enumerate(start_points[0:NUMBER_OF_START_POINTS]): 
        ...
        step_sizes = np.arange(7, 10.1, 0.5)/10
        time_limit = 200
        for step_size in step_sizes:
            cpp = BAstar(fakeprint, motion_planner, PointCloud(fakeprint, points= coverable_points), time_limit)
            cpp.name = cpp.name + " - " + str(step_size)
            cpp.step_size = step_size*ROBOT_SIZE
            cpp.visited_threshold = 0.5412*step_size
            perform_cpp(cpp, start_point, start_point_nr)
    ...
    with open('exp3.csv', 'w', newline='') as csvfile:
