...
def get_random_bastar_coverage(times, data):
    average_coverage = np.array([get_average(data, "Random BAstar", "coverage", time) for time in times])        
...
algorithms = ["Inward Spiral", "BAstar", "BAstar Variant", "1", "2", "3", "4", "5", "6" ,"7"]
line = ["b", "r:", "g--", "*m-.", ",m-.", "om-.", "<m-.", ">m-.", "sm-.", "+m-."]
labels = ["Inward Spiral", "BA*", "Curved BA*", "1", "2", "3", "4", "5", "6" ,"7"]
colors = [(0.0,0.0,1.0,0.3), (1.0,0.0,0.0,0.3), (0,1.0,0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3), (1.0,0.0,1.0,0.3)]  
...
def get_coverage_per_length(coverages, data):
    for alg_idx, algorithm in enumerate(algorithms):
...
def main():
    ...
    with open("experiment_official_2_w_randombastar_part_II.csv", newline='') as csvfile:
        data = csv.DictReader(csvfile)
        data = [row for row in data]
        times = np.array([int(row["time"]) for row in data])
        times = np.arange(0, max(times), 10)
        coverages = np.arange(0, 101, 1)
        #other_alg_results = print_means(data, 97.2)
    ...
    with open('experiment_official_2_w_randombastar_fix.csv', newline='') as csvfile:
        data = csv.DictReader(csvfile)
        data = [row for row in data]
        times = np.array([int(row["time"]) for row in data])
        times = np.arange(0, max(times), 10)
