...
class Spiral(CPPSolver):
    ...
    def get_cpp_path(self, start_point):
        ...
        while coverage < COVEREAGE_EFFICIENCY_GOAL:
            ...
            self.path = np.append( self.path, path_until_dead_zone, axis=0 )
            self.pcd.visit_path(path_until_dead_zone, ROBOT_RADIUS)

            #self.print("find_closest_free_pos")
            next_starting_point =  self.find_closest_free_pos_2(current_position, self.path)
            #next_starting_point = self.get_sorted_backtracking_list(self.path)[0]
            

            #self.print("next_starting_point: " + str(next_starting_point))
            #self.print("next_starting_point_angle: " + str(current_angle))
            if next_starting_point is False:
                break
            self.points_to_mark = np.append(self.points_to_mark, [next_starting_point], axis=0)
            #self.print("motion_planner")
            path_to_next_starting_point = self.motion_planner.Astar(current_position, next_starting_point)
    ...
    def find_closest_free_pos_2(self, start_position, path):
