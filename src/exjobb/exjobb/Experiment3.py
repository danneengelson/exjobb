...
def main():
    ...
    def perform_cpp(cpp, start_point, iter):
        path = cpp.get_cpp_path(start_point)
        length = 0
        rotation = 0
        coveage = -1
        for sec in np.arange(0, 200, 5):
            #print("sec" + str(sec))
            minimal = min(cpp.randombastar_stats_over_time, key = lambda i: abs(i['time'] - sec))

            time = sec
            if coveage < minimal["coverage"]:
                coveage = minimal["coverage"]
                length += get_length_of_path(minimal["path"])
                rotation += get_total_rotation(minimal["path"])
            #length = get_length_of_path(path[0:minimal["path_point"]])
            #rotation = get_total_rotation(path[0:minimal["path_point"]])
            stats = {
                "algorithm": cpp.name + str(iter),
                #"max_distance": cpp.max_distance,
                "max_distance": cpp.max_distance_part_II,
                "time": time,
                "coverage": coveage*100,
                "length": round(length),
    ...
    dummy_start = [-12.59000015,  11       ,    -5.29468489] 
    max_distances = [1,2,3,4,5,6,8,10, 12]
    for d_max in max_distances: 
        for i in range(1):
            print("d_max " + str(d_max) + " iteration: " + str(i))

            cpp = RandomBAstar(fakeprint, motion_planner, PointCloud(fakeprint, points= coverable_points), 200)
            #cpp.max_distance = d_max
            cpp.max_distance_part_II = d_max
            perform_cpp(cpp, dummy_start, i)

            #print(results)

    with open('experiment_part_II.csv', 'w', newline='') as csvfile:
        fieldnames = ['algorithm', 'max_distance', 'time', "coverage", "length", "rotation"]
