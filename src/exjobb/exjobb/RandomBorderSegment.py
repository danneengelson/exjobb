...
class RandomBorderSegment(Spiral):
    ...
    def __init__(self, print, motion_planner, starting_point, visited_waypoints, coverable_pcd, max_distance, step_size, visited_threshold, time_left):
        ...
        next_starting_point, current_angle = self.find_closest_wall(self.start, self.step_size)
        self.first_border_point = next_starting_point
        path_to_next_starting_point = self.motion_planner.Astar(self.start, next_starting_point)
        ...
        while True:
            ...
            if self.back_at_start_point(path_until_dead_zone) and iter > 2:
                break
    ...
    def back_at_start_point(self, path):
        ...
        self.print(closest)
        return closest < 3*self.step_size 
